...
files <- files[grep("genes.pi", files)]
...
d <- lapply(d, function(x) {
  colnames(x) <- c("gene", "n_variants", "prop_covered", "pi")
  x      
})
...
png("~/reciprocal_t/figures/gene.pi.hist.png", res=300, height=6, width=6, units="in")

par(mfrow = c(1, 1), mar=c(3, 3, 3, 1), mgp=c(3, 1, 0), las=0)

plot(density( d[['AAAA_F1_REP1']]$pi, na.rm=TRUE), 
    lwd=2, xaxt="n",yaxt="n", main="", xlim=c(-0.005, 0.04))
...
title(main="pi: genes", line=1, cex.lab=0.9)
...
png("~/reciprocal_t/figures/gene.pi01_quantile.png",width = 6, height = 6, res=300, units="in")
...
png("~/reciprocal_t/figures/gene.pi05_quantile.png",width = 6, height = 6, res=300, units="in")
ggplot(gdat_05, aes(x=line, y=pi, color=line)) +
    theme_bw() +
    stat_summary(fun.data="mean_se", mapping=aes(group=line), size=2.5,alpha = 0.5, show.legend=FALSE)+
    geom_jitter(width=0.1, size=3.5) +
    scale_color_manual(values=c("black", "firebrick3")) +
    ggtitle("gene: 0.05 quantile") +
    theme(axis.text.x=element_text(size=14),
        axis.title.x=element_blank()) +
    theme(legend.position="none", plot.title = element_text(hjust = 0.5))
dev.off()
...
one <- (merge(new[['AAAA_F1_REP1']], new[['AAAA_F1_REP2']], by="id"))[, c(-6)]
colnames(one) <- c("id", "gene", "variants_AAAA_F1_REP1", "prop_covered_AAAA_F1_REP1", "pi_AAAA_F1_REP1",
                    "variants_AAAA_F1_REP2", "prop_covered_AAAA_F1_REP2", "pi_AAAA_F1_REP2")

two <- (merge(one, new[['AAAA_F1_REP3']], by="id"))[,-c(9)]
colnames(two) <- c(colnames(one), "variants_AAAA_F1_REP3", "prop_covered_AAAA_F1_REP3", "pi_AAAA_F1_REP3")

three <- (merge(two, new[['AAAA_F1_REP4']], by="id"))[,-12]
colnames(three) <- c(colnames(two), "variants_AAAA_F1_REP4", "prop_covered_AAAA_F1_REP4", "pi_AAAA_F1_REP4")

four <- (merge(three, new[['HHHH_F1_REP1']], by="id"))[,-c(15)]
colnames(four) <- c(colnames(three), "variants_HHHH_F1_REP1", "prop_covered_HHHH_F1_REP1", "pi_HHHH_F1_REP1")

five <- (merge(four, new[['HHHH_F1_REP2']], by="id"))[,-c(18)]
colnames(five) <- c(colnames(four), "variants_HHHH_F1_REP2", "prop_covered_HHHH_F1_REP2", "pi_HHHH_F1_REP2")

six <- (merge(five, new[['HHHH_F1_REP3']], by="id"))[,-c(21)]
colnames(six) <- c(colnames(five), "variants_HHHH_F1_REP3", "prop_covered_HHHH_F1_REP3", "pi_HHHH_F1_REP3")

df.all <- (merge(six, new[['HHHH_F1_REP4']], by="id"))[,-c(24)]
colnames(df.all) <- c(colnames(six), "variants_HHHH_F1_REP4", "prop_covered_HHHH_F1_REP4", "pi_HHHH_F1_REP4")


# calc cutoff values

AAAA_REP1 <- which(df.all$pi_AAAA_F1_REP1 < quantile(df.all$pi_AAAA_F1_REP1, na.rm=TRUE, 0.05))
AAAA_REP2 <- which(df.all$pi_AAAA_F1_REP2 < quantile(df.all$pi_AAAA_F1_REP2, na.rm=TRUE, 0.05))
AAAA_REP3 <- which(df.all$pi_AAAA_F1_REP3 < quantile(df.all$pi_AAAA_F1_REP3, na.rm=TRUE, 0.05))
AAAA_REP4 <- which(df.all$pi_AAAA_F1_REP4 < quantile(df.all$pi_AAAA_F1_REP4, na.rm=TRUE, 0.05))
HHHH_REP1 <- which(df.all$pi_HHHH_F1_REP1 < quantile(df.all$pi_HHHH_F1_REP1, na.rm=TRUE, 0.05))
HHHH_REP2 <- which(df.all$pi_HHHH_F1_REP2 < quantile(df.all$pi_HHHH_F1_REP2, na.rm=TRUE, 0.05))
HHHH_REP3 <- which(df.all$pi_HHHH_F1_REP3 < quantile(df.all$pi_HHHH_F1_REP3, na.rm=TRUE, 0.05))
HHHH_REP4 <- which(df.all$pi_HHHH_F1_REP4 < quantile(df.all$pi_HHHH_F1_REP4, na.rm=TRUE, 0.05))
