...
#make empty array
num_lines = sum(1 for line in open('/users/r/b/rbrennan/reciprocal_t/analysis/pca_loadings_goenrich.txt'))-1

go_df = np.empty(shape=(num_lines,2), dtype = object)

i=0 # start counter

with open('/users/r/b/rbrennan/reciprocal_t/analysis/pca_loadings_goenrich.txt') as master_file:
...
np.savetxt('/users/r/b/rbrennan/reciprocal_t/analysis/pcLoadings_GOterms.out', go_df,fmt='%5s', delimiter='\t')
