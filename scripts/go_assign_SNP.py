...
#make empty array
num_lines = sum(1 for line in open('/users/r/b/rbrennan/reciprocal_t/analysis/GO_enrich/snp_SIG_all.txt'))-1

go_df = np.empty(shape=(num_lines,2), dtype = object)

ict=0 # start counter

with open('/users/r/b/rbrennan/reciprocal_t/analysis/GO_enrich/snp_SIG_all.txt') as master_file:
...
np.savetxt('/users/r/b/rbrennan/reciprocal_t/analysis/GO_enrich/snp_all_GOterms.out', go_df,fmt='%s', delimiter='\t')
